...
function InstallButton() {
  ...
  useEffect(() => {
    const browserType = (function () {
      ...
      switch (true) {
        case window.navigator.brave != undefined && window.navigator.brave.isBrave.name == "isBrave":
          return "Brave";
        case test(/edg/i):
          return "Edge";
        case test(/trident/i):
  ...
  return (
    <div className="btn">
      <a href={extUrl} target="_blank" rel="noreferrer">
          <button className="Add-to-chrome button-box  flex  cursor-pointer rounded-[4px] p-2.5">
          <Chrome />  
            <p className="h2 secondary_text_color  pl-2.5 font-bold text-white">
             Add to {browser} <span style={{fontSize:"12px",color: "rgb(255 255 255)",fontWeight: "600"}}>it &apos; s free</span>
            </p>
          </button>
      </a>
    </div>
